...
import { BookOpen, Bell, ArrowRight, ArrowLeft } from "lucide-react";
...
interface AnnouncementCoursesContentProps {
    id: string;
    courses: Course[];
}

export default function AnnouncementCoursesContent({ id, courses }: AnnouncementCoursesContentProps) {
    return (
        <motion.div
            className="space-y-6"
        ...
        >
            <motion.div variants={itemVars} className="flex items-center justify-between">
                <div>
                    ...
                    <h2 className="text-3xl font-bold text-gray-800 tracking-tight">Course Announcements</h2>
                ...
                {courses.length > 0 ? (
                    courses.map((course) => (
                        ...
                        >
                            <motion.div
                                variants={itemVars}
                                className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-orange-200 transition-all h-full flex flex-col hover:-translate-y-1 relative"
                            >
                                <div className="h-2 bg-gradient-to-r from-orange-500 to-amber-500"></div>
                                <div className="p-6 flex-1 flex flex-col">
                                    <div className="flex justify-between items-start mb-4">
                                        <span className="px-2 py-1 bg-orange-50 text-orange-700 text-xs font-bold rounded uppercase tracking-wider border border-orange-100">
                                            {course.courseNo}
                                        </span>
                                    ...
                                    {course.announcements && course.announcements.length > 0 && (
                                        <div className="mt-2 text-xs text-gray-500 bg-gray-50 p-2 rounded-lg border border-gray-100 line-clamp-2 italic">
                                    ...
                                    <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between text-sm">
                                        ...
                                        <div className="h-8 w-8 rounded-full flex items-center justify-center bg-gray-50 group-hover:bg-orange-600 group-hover:text-white transition-all">
                    ...
                    <motion.div variants={itemVars} className="col-span-full text-center py-16 bg-white rounded-2xl border border-dashed border-gray-200">
                        <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-orange-50 mb-6 group">
                            <BookOpen className="h-8 w-8 text-orange-400 group-hover:text-orange-600 transition-colors" />
                        </div>
