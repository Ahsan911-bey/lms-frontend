...
interface MarkAttendanceCoursesContentProps {
    id: string;
    courses: Course[];
}

export default function MarkAttendanceCoursesContent({ id, courses }: MarkAttendanceCoursesContentProps) {
    return (
        <motion.div
            className="space-y-6"
            ...
            <motion.div variants={itemVars} className="flex items-center justify-between">
                <div>
                    <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
                        <Link href={`/teachers/${id}/course-actions/attendance`} className="hover:text-blue-600 hover:underline flex items-center gap-1 transition-colors">
                ...
                {courses.length > 0 ? (
                    courses.map((course) => (
                        ...
                        >
                            <motion.div
                                variants={itemVars}
                                className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-emerald-200 transition-all h-full flex flex-col hover:-translate-y-1 relative"
                            >
                                <div className="h-2 bg-gradient-to-r from-emerald-500 to-teal-400"></div>
                                <div className="p-6 flex-1 flex flex-col">
                                    <div className="flex justify-between items-start mb-4">
                                        <span className="px-2 py-1 bg-emerald-50 text-emerald-700 text-xs font-bold rounded uppercase tracking-wider border border-emerald-100">
                                            {course.courseNo}
                                        </span>
                                    ...
                                    <div className="mt-auto pt-4 border-t border-gray-50 flex items-center justify-between text-sm">
                                        ...
                                        <div className="h-8 w-8 rounded-full flex items-center justify-center bg-gray-50 group-hover:bg-emerald-600 group-hover:text-white transition-all">
                    ...
                    <motion.div variants={itemVars} className="col-span-full text-center py-16 bg-white rounded-2xl border border-dashed border-gray-200">
                        <div className="inline-flex h-16 w-16 items-center justify-center rounded-full bg-emerald-50 mb-6 group">
                            <BookOpen className="h-8 w-8 text-emerald-400 group-hover:text-emerald-600 transition-colors" />
                        </div>
