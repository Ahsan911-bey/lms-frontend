...
interface CheckAttendanceContentProps {
    id: string;
    courses: Course[];
}

export default function CheckAttendanceContent({ id, courses }: CheckAttendanceContentProps) {
    return (
        <motion.div
            className="space-y-6"
        ...
        >
            <motion.div variants={itemVars} className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Choose a course to check attendance for.</p>
                </div>
            </motion.div>
            ...
            <motion.div variants={containerVars} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-4">
                {courses.length > 0 ? (
                    courses.map((course) => (
                        <Link
                            key={course.id}
                            href={`/teachers/${id}/course-actions/attendance/check/${course.courseNo}`}
                            className="block group"
                        >
                            <motion.div
                                variants={itemVars}
                                className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-blue-200 transition-all h-full flex flex-col hover:-translate-y-1 relative"
                            >
